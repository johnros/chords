...
(a) casting the data into the RDS file format,
...
We start by simulating $10$ RDS samples.
The population consists of $4$ observed degree classes $(2,5,10,20)$, with $1,000$ individuals per degree. 
Formally, $N_k=1,000$ for $k=2,5,10,20$, so that $N=4,000$.

<<r file format>>=
library(magrittr)
library(chords)
dk <- c(2, 5, 10, 20) # four degree classes
...
sample.length <- 1e3 # the total sample size
nsims <- 1e1
simlist <- list()
for(i in 1:nsims){
...
The file format adheres to the RDS conventions as described in \url{http://www.respondentdrivensampling.org/reports/RDSAT_7.1-Manual_2012-11-25.pdf}.
In particular note the \code{NS1} variable which encodes the (self reported) degree of each recruitee, 
\code{refCoupNum} for the recruiter's coupon number, 
\code{coup1}-\code{coup3} for the coupons given upon recruitment, 
and \code{interviewDt} for the time of recruitment. 
The scale of \code{interviewDt} is immaterial since the initial time will be later subtracted, and any rescaling will merely change the time units of the baseline recruitment rate $\beta_k$. 
We now ready to call the \code{rds-object} constructor:
<<>>=
#rds.object<- initializeRdsObject(brazil)
@
\code{rds.object} will now hold all the information needed for the estimation, and an empty slot for future estimates. 
The default maximum likelihood estimates are now computed:
<<>>=
